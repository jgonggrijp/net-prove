...
\end{conjecture}
\noindent\emph{Motivation.}\hspace{6pt}Given that there are generally few \hs/Term/s for a proof net, the \hs/SubnetGraph/ tree might contain only a few branching points in most cases. In that case, the majority of \hs/Subnet/s belongs to subgraphs where the branching factor $b$ from Theorem~\ref{thm:exponential} degrades to 1. The overall complexity then becomes $N$ times something that is not much larger, and probably even smaller, than $N$.

\begin{theorem}
    The space complexity of \hs/LG.SubnetGraph.validExtensions/ is $\Theta(N)$.
\end{theorem}
\begin{proof}
