...
For a practical implementation, it is more convenient to start with a rich datastructure and ignore any unneeded information later on, than to start with a minimal datastructure and commit oneself to augmenting or converting the data when more information is required. Given that all necessary information to create a full-blown composition graph is available from the start, we decided to work with composition graphs exclusively. Our implementation, \haskell/CompositionGraph/, is a fairly typical linked (hyper)graph, in which identifiers are mapped to node containers, \haskell/NodeInfo/, which contain a formula, a term, an optional uplink and an optional downlink. Please refer to \coderef{src/LG/Graph.hs} for the details.


\subsection{Term computation}

As implemented, term computation from a \haskell/CompositionGraph/ takes two steps. First all rooted \haskell/Subnet/s, i.e. subgraphs containing only tensor and substitution links, are identified and collected in a new graph datastructure, \haskell/SubnetGraph/. In our implementation, this includes solitary nodes that don't have tensor or substitution links attached. The algorithm is summarized as follows.
\begin{itemize}
    \item While any unvisited nodes remain:\begin{enumerate}
        \item Pick any unvisited node and add it to a new \haskell/Subnet/.
        \item Consider attached \haskell/Link/s, if any:\label{alg:extract-recurse}\begin{itemize}
            \item If tensor or substitution, add the neighbouring nodes to the current \haskell/Subnet/ and recurse with those nodes into \ref{alg:extract-recurse}, taking care not to follow the same link again.
            \item Otherwise, define a boundary of the current \haskell/Subnet/.
        \end{itemize}
        \item Add the current \haskell/Subnet/ to the \haskell/SubnetGraph/ and regard all its nodes as visited.
    \end{enumerate}
\end{itemize}
